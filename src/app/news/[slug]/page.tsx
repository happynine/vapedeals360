...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    if (slug) {
      Promise.all([
        fetch(`/api/content-pages?slug=${slug}&language=${language}`).then(r => r.json()),
        fetch('/api/social-links').then(r => r.json()),
      ]).then(([contentData, socialData]) => {
        if (contentData.success) setPage(contentData.data);
            ...
            <div>
              ...
              {socialLinks.length > 0 && (
                <div className="flex items-center gap-3">
                  {socialLinks.filter((l: SocialLink) => l.is_active).sort((a: SocialLink, b: SocialLink) => a.sort_order - b.sort_order).map((link: SocialLink) => (
